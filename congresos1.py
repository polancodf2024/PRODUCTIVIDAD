...
def display_congreso_info(data, investigator_name):
    ...
    st.markdown("**Línea de investigación**")
    st.markdown(f"🔍 {data['linea_investigacion']}")

def main():
    st.set_page_config(
        page_title="Captura Congresos",
        page_icon="🎤",
        layout="centered"
    )
    ...
    if Path(csv_filename).exists():
        try:
            congresos_df = pd.read_csv(csv_filename, encoding='utf-8-sig', dtype={'economic_number': str})
            congresos_df['economic_number'] = congresos_df['economic_number'].astype(str).str.strip()

            # Asegurar que los campos SNI, SII y nombramiento existan y tengan valores
            if 'sni' not in congresos_df.columns:
                congresos_df['sni'] = sni
            else:
                congresos_df['sni'] = congresos_df['sni'].fillna(sni)

            if 'sii' not in congresos_df.columns:
                congresos_df['sii'] = sii
            else:
                congresos_df['sii'] = congresos_df['sii'].fillna(sii)

            if 'nombramiento' not in congresos_df.columns:
                congresos_df['nombramiento'] = nombramiento
            else:
                congresos_df['nombramiento'] = congresos_df['nombramiento'].fillna(nombramiento)

            # Asegurar que el campo 'estado' exista
            if 'estado' not in congresos_df.columns:
                congresos_df['estado'] = 'A'
            else:
                # Limpiar valores vacíos/nulos en el campo estado
                congresos_df['estado'] = congresos_df['estado'].fillna('A').str.strip().replace('', 'A')
        except Exception as e:
    ...
    if not congresos_df.empty:
        st.subheader(f"📋 Congresos registrados para {economic_number}")
        st.info("""
        ...
        *Nota:* El botón **"Confirmar baja de registros"** aparecerá automáticamente cuando haya registros marcados con 'X'.
        ...
        if not edited_df.equals(congresos_df[columnas_mostrar]):
            # Actualizar el estado en el DataFrame original
            congresos_df['estado'] = edited_df['estado']

            # Identificar registros marcados para borrar
            registros_a_borrar = congresos_df[congresos_df['estado'] == 'X']
            ...
            if not registros_a_borrar.empty:
                ...
                # Mostrar solo el botón de confirmar baja
                if st.button("🗑️ Confirmar baja de registros", type="primary"):
                    # Filtrar solo los registros activos (estado 'A')
                    congresos_df = congresos_df[congresos_df['estado'] == 'A'].copy()

                    # Guardar cambios en el archivo
                    congresos_df.to_csv(csv_filename, index=False, encoding='utf-8-sig')
    ...
    st.divider()
    if st.radio("¿Desea registrar un nuevo congreso?", ["No", "Sí"], index=0) == "Sí":
        ...
        with st.form("nuevo_congreso", clear_on_submit=True):
            ...
            linea_investigacion = st.multiselect(
                ...
                options=list(LINEAS_INVESTIGACION.keys()),
