...
def main():
    ...
    try:
        ...
        if not Path(csv_filename).exists():
            ...
            if 'estado' not in capitulos_df.columns:
                ...
                capitulos_df['estado'] = capitulos_df['estado'].fillna('A').apply(lambda x: 'A' if x.strip() not in ['A', 'X'] else x.strip())

        filtered_records = capitulos_df[capitulos_df['economic_number'] == economic_number]

        if not filtered_records.empty:
            st.subheader(f"📋 Libros y capítulos registrados para {economic_number}")
            
            # Nota sobre el campo Estado
            st.info("""
            **Nota sobre el campo Estado:**  
            - 'A' = Registro activo (valor por defecto)  
            - 'X' = Registro marcado para borrar  
            Los registros marcados con 'X' se eliminarán al guardar nuevos cambios.
            """)
            
            # Mostrar tabla editable con el campo Estado
            edited_df = st.data_editor(
                filtered_records[['titulo_libro', 'titulo_capitulo', 'tipo_participacion', 'year', 'estado']],
                column_config={
            ...
            if not edited_df.equals(filtered_records[['titulo_libro', 'titulo_capitulo', 'tipo_participacion', 'year', 'estado']]):
                # Actualizar el DataFrame original con los cambios
                capitulos_df.update(edited_df)
    ...
    st.subheader("📝 Información del capítulo")
    
    # Campos de entrada manual para libro/capítulo
    ...
    autor_principal = st.text_input("👤 Nombre completo del autor principal del libro:", key="autor_principal")
    ...
    titulo_capitulo = st.text_area("📄 Título del capítulo:", height=100, key="titulo_capitulo")
    editorial = st.text_input("🏢 Editorial:", key="editorial")
    coautores_secundarios = st.text_area("👥 Coautores del capítulo, separados por ';'):", key="coautores_secundarios")
    ...
    paginas = st.text_input("📚 Número de páginas del capítulo:", key="paginas")
    ...
    st.markdown("**Información del capítulo**")
    st.write(f"📖 Título del libro: {titulo_libro}")
    if titulo_capitulo:
        st.write(f"📄 Título del capítulo: {titulo_capitulo}")
    ...
    if coautores_secundarios:
        st.markdown("👥 Coautores del capitulo:")
