...
def crear_nombres_txt(nombres, apellidos, numero_economico):
    """Crea el archivo nombres{numero_economico}.txt con el formato adecuado."""
    variantes_nombres = generar_variantes_nombres(nombres, apellidos)
    afiliaciones = ["Cardiología[Affiliation]", "Cardiology[Affiliation]"]

    nombre_archivo = f"nombres{numero_economico}.txt"
...
def procesar_nombres(numero_economico):
    """Lee y procesa los nombres y afiliaciones en el archivo 'nombres{numero_economico}.txt'."""
    if not numero_economico:
        st.error("El numero_economico no está definido. Por favor, verifica.")
        return False

    file_name = f"nombres{numero_economico}.txt"
    ...
    if ocurrencias:
        ...
        if st.button("Actualización"):
            ...
            with st.spinner("Procesando, por favor espera..."):
                ...
                st.warning("Iniciando procesamiento del archivo remoto...")
...
def ejecutar_pubmed():
    ...
    if st.button("Validar Clave", key="validar_clave"):
        if "clave_generada" in st.session_state and clave_ingresada == st.session_state.clave_generada:
            ...
            st.warning("Revise la carpeta Spam si no encuentra el correo enviado.")
            ...
            with st.spinner("Procesando, por favor espera..."):
                if nombres and apellidos and numero_economico and correo:
                    crear_nombres_txt(nombres, apellidos, numero_economico)
                    if procesar_nombres(numero_economico):
                        st.session_state.mostrar_ocurrencias = True
                        st.warning("La producción que se muestra corresponde al año 2024 en adelante.")
